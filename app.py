...
# 2. 註冊 Windows / 雲端環境中文字體（解決跨平台缺字亂碼）
font_path = r"C:\Windows\Fonts\msjh.ttc"  # 本地 Windows 預設
if os.path.exists(font_path):
    pdfmetrics.registerFont(TTFont("MSJH", font_path))
    font_name = "MSJH"
else:
    # 備用方案：如果部署到 Streamlit 雲端伺服器 (Linux)，可回退至 Helvetica 或系統預設
    # 在實際部署時，若雲端缺中文字型，可將 msjh.ttc 檔案一併上傳到 GitHub 專案目錄下直接讀取
    font_name = "Helvetica"
...
if uploaded_file is not None:
    try:
        ...
        if st.button("🚀 執行批次生成並打包下載", type="primary"):
            ...
            with st.spinner("正在為您精心繪製標籤並壓縮打包中，請稍候..."):
                ...
                with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                    ...
                    # 4. 初始化您原創的 ReportLab 畫布參數 (1000x400 px -> 750x300 points)
                    ...
                    for index, row in df.iterrows():
                        ...
                        # 將繪製完成的單一 PDF 寫入 ZIP 壓縮包中，檔名依照 Excel 欄位
                        pdf_buffer.seek(0)
        ...
        st.error(f"处理過程中發生失敗，請檢查檔案格式與表頭：\n{str(e)}")